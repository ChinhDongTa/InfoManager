...
public class Identities : EndpointGroupBase
{
    ...
    public override void Map(RouteGroupBuilder group)
    {
        ...
        api.MapPut(UpdateRoleAsync, "roles/{id}");
    ...
    public async Task<IResult> GetUserByIdAsync(string id, IIdentityService identityService, CancellationToken ct)
    {
        ...
        return result.Succeeded ? TypedResults.Ok(result.Value) : TypedResults.BadRequest(result.Errors?.ToLine() ?? "Not found");
    }
    ...
    public async Task<IResult> DeleteUserAsync(string id, IIdentityService identityService, CancellationToken ct)
    {
        ...
        return result.Succeeded ? TypedResults.NoContent() : TypedResults.BadRequest(result.Errors?.ToLine() ?? "Delete failed");
    }
    ...
    public async Task<IResult> RemoveFromRoleAsync([FromBody] RoleActionDto request, IIdentityService identityService, CancellationToken ct)
    {
        ...
        var result = await identityService.RemoveFromRoleAsync(request, ct);
        return result.Succeeded ? TypedResults.NoContent() : TypedResults.BadRequest(result.Errors?.ToLine() ?? "Operation failed");
    }
    ...
    public async Task<IResult> AddToRoleAsync([FromBody] RoleActionDto request, IIdentityService identityService, CancellationToken ct)
    {
        ...
        var result = await identityService.AddToRoleAsync(request, ct);
        return result.Succeeded ? TypedResults.NoContent() : TypedResults.BadRequest(result.Errors?.ToLine() ?? "Operation failed");
    }
    ...
    public async Task<IResult> CreateRoleAsync([FromBody] CreateRoleDto request, IIdentityService identityService, CancellationToken ct)
    {
        ...
        if (!result.Succeeded)
            return TypedResults.BadRequest(result.Errors?.ToLine() ?? "Create failed");
    ...
    {
        if (id != request.Id)
            return TypedResults.BadRequest(ErrorHelpers.GetErrorMismatch("Role Id"));
        var result = await identityService.UpdateRoleAsync(id, request, ct);
        return result.Succeeded ? TypedResults.NoContent() : TypedResults.BadRequest(result.Errors?.ToLine() ?? "Update failed");
    }
    ...
    public async Task<IResult> GetRoleNamesAsync(IIdentityService identityService, CancellationToken ct)
    {
        ...
        return result.Succeeded ? TypedResults.Ok(result.Value) : TypedResults.BadRequest(result.Errors?.ToLine() ?? "Query failed");
    }
    ...
    public async Task<IResult> RegisterAsync(LoginRequest request, IIdentityService identityService, CancellationToken ct)
    {
        ...
        var result = await identityService.CreateUserAsync(request, ct);
        if (!result.Succeeded)
