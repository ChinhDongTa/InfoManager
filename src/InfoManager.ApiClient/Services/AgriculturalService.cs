...
public class AgriculturalService(IAgriculturalApi api) : IAgriculturalService
{
    ...
    public async Task<ApiResult<CropDto?>> GetCropByIdQueryAsync(string id, CancellationToken ct = default)
   ...
   => await ApiResponseHandler.HandleAsync(await api.GetCropPlantingsAsync(pageNumber, pageSize, ct));
    ...
    public async Task<ApiResult<PaginatedList<CropSummaryDto>>> GetCropsQueryAsync(int pageNumber, int pageSize, CancellationToken ct = default)
   ...
   => await ApiResponseHandler.HandleAsync(await api.GetCropVarietiesAsync(pageNumber, pageSize, ct));
    ...
    public async Task<ApiResult<GrowthStageAlertDto?>> GetGrowthStageAlertByIdQueryAsync(string id, CancellationToken ct = default)
   => await ApiResponseHandler.HandleAsync(await api.GetGrowthStageAlertByIdQueryAsync(id, ct));

    public async Task<ApiResult<PaginatedList<GrowthStageAlertSummaryDto>>> GetGrowthStageAlertsQueryAsync(int pageNumber, int pageSize, CancellationToken ct = default)
   => await ApiResponseHandler.HandleAsync(await api.GetGrowthStageAlertsQueryAsync(pageNumber, pageSize, ct));

    public async Task<ApiResult<PaginatedList<GrowthStageSummaryDto>>> GetGrowthStagesQueryAsync(int pageNumber, int pageSize, CancellationToken ct = default)
   => await ApiResponseHandler.HandleAsync(await api.GetGrowthStagesQueryAsync(pageNumber, pageSize, ct));
    ...
    public async Task<ApiResult<PaginatedList<CropSummaryDto>>> SearchCropsQueryAsync(SearchCropRequest request, CancellationToken ct = default)
   ...
   => await ApiResponseHandler.HandleAsync(await api.SearchCropVarietiesAsync(request.Term,
    ...
    public async Task<ApiResult<PaginatedList<GrowthStageAlertSummaryDto>>> SearchGrowthStageAlertsQueryAsync(SearchGrowthStageAlertRequest request, CancellationToken ct = default)
    ...
    public async Task<ApiResult<PaginatedList<GrowthStageSummaryDto>>> SearchGrowthStagesQueryAsync(SearchGrowthStageRequest request, CancellationToken ct = default)
