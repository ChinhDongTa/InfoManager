...
{
    public async Task<Result<CropScheduleDto?>> Handle(GetCropScheduleByIdQuery request, CancellationToken cancellationToken)
    {
        return await context.CropSchedules.Where(x=>x.Id == request.Id)
            .ToCropScheduleDto()
            .SingleOrNotFoundAsync(nameof(CropSchedule), request.Id, cancellationToken);
    }
}
