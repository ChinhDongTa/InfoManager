...
{
    public async Task<Result<GrowthStageDto?>> Handle(GetGrowthStageByIdQuery request, CancellationToken cancellationToken)
    {
        var result = await context.GrowthStages
            .Where(x => x.Id == request.Id)
            .ToGrowthStageDto()
            .SingleOrNotFoundAsync(nameof(GrowthStage), request.Id, cancellationToken);
        return result;
    }
}