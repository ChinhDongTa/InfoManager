...
{
    public async Task<Result<GrowthStageAlertDto?>> Handle(GetGrowthStageAlertByIdQuery request, CancellationToken ct)
    {
        var result = await context.GrowthStageAlerts
            .Where(x => x.Id == request.Id)
            .ToGrowthStageAlertDto()
            .SingleOrNotFoundAsync(nameof(GrowthStageAlert), request.Id, ct);
        return result;
    }
}