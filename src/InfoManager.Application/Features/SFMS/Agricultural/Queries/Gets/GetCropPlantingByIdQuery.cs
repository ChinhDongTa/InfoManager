...
{
    public async Task<Result<CropPlantingDto?>> Handle(GetCropPlantingByIdQuery request, CancellationToken cancellationToken)
    {
        var result = await context.CropPlantings
            .Where(x => x.Id == request.Id)
            .ToCropPlantingDto()
            .SingleOrNotFoundAsync(nameof(CropPlanting), request.Id, cancellationToken);
        return result;
    }
}