...
{
    public async Task<Result<HREmployeeDto?>> Handle(GetHREmployeeByIdQuery request, CancellationToken cancellationToken)
    {
        var result = await context.HREmployees
            .Where(x => x.Id == request.Id)
            .ToHREmployeeDto()
            .SingleOrNotFoundAsync(nameof(HREmployee), request.Id, cancellationToken);
        return result;
    }
}
