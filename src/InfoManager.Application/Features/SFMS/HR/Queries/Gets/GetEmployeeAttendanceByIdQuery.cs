...
{
    public async Task<Result<EmployeeAttendanceDto?>> Handle(GetEmployeeAttendanceByIdQuery request, CancellationToken ct)
    {
        var result = await context.EmployeeAttendances
            .Where(x => x.Id == request.Id)
            .ToEmployeeAttendanceDto()
            .SingleOrNotFoundAsync(nameof(EmployeeAttendance), request.Id, ct);
        return result;
    }
}