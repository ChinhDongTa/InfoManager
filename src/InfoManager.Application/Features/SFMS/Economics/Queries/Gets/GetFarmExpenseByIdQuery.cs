...
{
    public async Task<Result<FarmExpenseDto?>> Handle(GetFarmExpenseByIdQuery request, CancellationToken cancellationToken)
    {
        var result = await context.FarmExpenses
            .Where(x => x.Id == request.Id)
            .ToFarmExpenseDto()
            .SingleOrNotFoundAsync(nameof(FarmExpense), request.Id, cancellationToken);
        return result;
    }
}