...
public class DeleteCustomerPaymentCommandHandler : BaseDeleteCommandHandler<DeleteCustomerPaymentCommand, CustomerPayment>
{
    ...
    protected override async Task<CustomerPayment?> GetEntityAsync(DeleteCustomerPaymentCommand request, CancellationToken ct)
        => await Context.CustomerPayments.FindAsync([request.Id], ct);
}