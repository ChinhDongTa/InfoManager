...
{
    public async Task<Result<PaginatedList<CustomerCareSummaryDto>>> Handle(SearchCustomerCaresQuery request, CancellationToken cancellationToken)
    {
    ...
    static IQueryable<CustomerCare> BuildSearchQuery(IQueryable<CustomerCare> query, SearchCustomerCaresQuery request)
    {
