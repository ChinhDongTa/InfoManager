...
public class DeleteHarvestPlanCommandHandler : BaseDeleteCommandHandler<DeleteHarvestPlanCommand, HarvestPlan>
{
    ...
    protected override async Task<HarvestPlan?> GetEntityAsync(DeleteHarvestPlanCommand request, CancellationToken cancellationToken)
  => await Context.HarvestPlans.FindAsync(request, cancellationToken);
}
