...
public class DeleteCropCycleCommandHandler : BaseDeleteCommandHandler<DeleteCropCycleCommand, CropCycle>
{
    ...
    protected override async Task<CropCycle?> GetEntityAsync(DeleteCropCycleCommand request, CancellationToken ct)
  => await Context.CropCycles.FindAsync(request, ct);
}