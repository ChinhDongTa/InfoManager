...
{
    public async Task<Result<HarvestPlanDto?>> Handle(GetHarvestPlanByIdQuery request, CancellationToken cancellationToken)
    {
        var result = await context.HarvestPlans
            .Where(x => x.Id == request.Id)
            .ToHarvestPlanDto()
            .SingleOrNotFoundAsync(nameof(HarvestPlan), request.Id, cancellationToken);
        return result;
    }
}