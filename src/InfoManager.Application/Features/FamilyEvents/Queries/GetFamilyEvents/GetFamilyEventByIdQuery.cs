...
{
    public async Task<Result<FamilyEventDto?>> Handle(GetFamilyEventByIdQuery request, CancellationToken cancellationToken)
    {
        ...
        var familyEvent = await Context.FamilyEvents
            ...
            .ToQueryDto()
            .SingleOrNotFoundAsync(nameof(FamilyEvent), request.Id, cancellationToken);
