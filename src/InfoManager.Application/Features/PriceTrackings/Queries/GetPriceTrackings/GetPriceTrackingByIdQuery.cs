...
{
    public async Task<Result<PriceTrackingDto?>> Handle(GetPriceTrackingByIdQuery request, CancellationToken cancellationToken)
    {
        var result = await context.PriceTrackings
            .Where(x => x.Id == request.Id)
            .ToPriceTrackingDto()
            .SingleOrNotFoundAsync(nameof(PriceTracking), request.Id, cancellationToken);
        return result;
    }
}
